📌 BUSINESS REQUIREMENT 
🎯 Objective
The company wants to analyze investment performance across different asset types (Equity, Bond, Real Estate, Gold, Mutual Fund, Crypto) between 2015–2025 to:
- Identify most beneficial and least beneficial assets
- Compare ROI vs Risk
- Analyze performance across time, regions, and market conditions

📊 KPIs to Answer the Requirement
- Total Investment Amount
- Total Profit
- Average ROI %
- Risk Score
- Volatility Index
- Asset Performance Trend (Yearly)
- Best & Worst Asset
- Market Condition Impact

🧱 DATA 
Columns used:
- Date, Year, Month, Quarter
- Asset_Type
- Investment_Amount
- Profit_Amount
- ROI_Percent
- Risk_Score
- Volatility_Index
- Region
- Market_Condition

📌 CONCLUSION: Strategic Investment Performance & Risk Optimization
Based on the analysis , the study evaluated multiple asset classes including Equity, Bonds, Real Estate, Gold, Mutual Funds, and Crypto using key performance indicators such as ROI, Profit, Risk Score, Volatility, Liquidity, Market Conditions, and Cost Factors.

🔍 Key Findings
- Equity and Mutual Funds emerged as the most balanced and beneficial assets, delivering consistently high ROI with moderate risk, especially over long holding periods.
- Crypto showed the highest returns, but with very high volatility and risk, making it suitable only for risk-tolerant investors.
- Bonds provided the lowest risk and volatility, but also generated the lowest returns, making them ideal for capital preservation rather than growth.
- Real Estate performed strongly in long-term investments, particularly during stable and bull market conditions.
- Gold acted as a defensive asset, offering stable returns during market uncertainty
